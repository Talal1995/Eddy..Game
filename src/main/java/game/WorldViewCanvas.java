...
//	public double tileSizeHeight;
	...
	public WorldViewCanvas(Room owner)
	{
		...
		hideMapCanvas = new Canvas(viewSizeHeight, viewSizeHeight);
	}
	...
	public synchronized void drawInterFeasibility()
	{
		if(interFeasibilityCanvas.isVisible())
		{
	    	...
	    	MapRenderer.getInstance().drawRoomPath(interFeasibilityCanvas.getGraphicsContext2D(), 
	    			...
	    			Color.RED);
	    	System.out.println(owner.nonInterFeasibleTiles.getArea());
		}
	}
	public synchronized void drawMapHidden(){

		interFeasibilityCanvas.getGraphicsContext2D().clearRect(0, 0, interFeasibilityCanvas.getWidth(), interFeasibilityCanvas.getHeight());
		MapRenderer.getInstance().hideRoomPath(interFeasibilityCanvas.getGraphicsContext2D(),
				owner.matrix,
				owner.nonInterFeasibleTiles,
				Color.BLACK);
	}
	public synchronized void drawMapPercentage(HashMap<UUID, int[][]> heatMap, Room owner){

		interFeasibilityCanvas.getGraphicsContext2D().clearRect(0, 0, interFeasibilityCanvas.getWidth(), interFeasibilityCanvas.getHeight());
		MapRenderer.getInstance().showHeatMap(interFeasibilityCanvas.getGraphicsContext2D(),
				owner.matrix,
				owner.nonInterFeasibleTiles,
				Color.RED, heatMap, owner);
	}
	public synchronized void drawMapVisible(){
		interFeasibilityCanvas.getGraphicsContext2D().clearRect(0, 0, interFeasibilityCanvas.getWidth(), interFeasibilityCanvas.getHeight());
		MapRenderer.getInstance().showRoomPath(interFeasibilityCanvas.getGraphicsContext2D(),
				owner.matrix,
				owner.nonInterFeasibleTiles,
				Color.WHITE);
	}
	public synchronized void drawPlayerDeath(HashMap<UUID, int[][]> heatMap, Room owner, int x, int y){

		interFeasibilityCanvas.getGraphicsContext2D().clearRect(0, 0, interFeasibilityCanvas.getWidth(), interFeasibilityCanvas.getHeight());
		MapRenderer.getInstance().drawPlayerDeath(interFeasibilityCanvas.getGraphicsContext2D(),
				owner.matrix,
				owner.nonInterFeasibleTiles,
				Color.RED, heatMap, owner, x, y);
	}
    /**
