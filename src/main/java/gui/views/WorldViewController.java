...
//			ani.getView().setFitHeight(50);
		...
		arrangeControls(startGameBtn, -30, 300, 200, 50);
...
//		buttonPane.getChildren().add(getPickInitBtn());
		...
		buttonPane.getChildren().add(startGameBtn);
...
//		removeRoomBtn.setText("REMOVE ROOM");
		...
		startGameBtn.setText("Start game / Get Data");
...
//		
		...
		startGameBtn.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent e) {
				...
				router.postEvent(new StartDungeon(dungeon, -1, widthField.getValue(), heightField.getValue()));
				Stage scene = (Stage) startGameBtn.getScene().getWindow();
				scene.close();
			}
