...
//				ctx.drawImage(image, i * tileSize, j * tileSize, tileSize, tileSize);
	...
	public synchronized void hideRoomPath(
			GraphicsContext ctx,
			int[][] matrix,
			Bitmap path,
			Color c) {
	...
	public synchronized void showHeatMap(
			GraphicsContext ctx,
			int[][] matrix,
			Bitmap path,
			Color c, HashMap<UUID, int[][]> heatMap, Room owner) {
		...
		drawHeatMap(ctx, path, c, tileSize, heatMap, owner);
	}
	public synchronized void drawPlayerDeath(
			GraphicsContext ctx,
			int[][] matrix,
			Bitmap path,
			Color c, HashMap<UUID, int[][]> heatMap, Room owner, int x, int y) {
		...
		drawStroke(ctx,pnt,Color.BLACK,tileSize);
	}
	public synchronized void showRoomPath(
			GraphicsContext ctx,
			int[][] matrix,
			Bitmap path,
			Color c) {
...
//			}
	...
	private void drawHeatMap(GraphicsContext ctx, Bitmap b, Color c, double pWidth, HashMap<UUID, int[][]> heatMap, Room owner){
		double max = 0;
		int[][] roomCounter = heatMap.get(owner.specificID);
		for(int[][] hMap : heatMap.values()){
			...
			for (int i = 0; i < roomCounter.length; i++) {
				for (int j = 0; j < roomCounter[i].length; j++) {
					if(max > 0 && roomCounter[i][j] > 0) {
						double steps = roomCounter[i][j];

						double percentage = steps / max;
						patternOpacity = percentage;
						int index = i * roomCounter[i].length + j;
						drawPoint(ctx, b.getPoint(index), c, pWidth);
					}
				}
			}
	}
	...
	private void drawStroke(GraphicsContext ctx, Point p, Color c, double width) {
		ctx.setStroke(c);
		ctx.setLineWidth(3);
